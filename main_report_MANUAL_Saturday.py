...
def get_jira_info(login, pwd):
    ...
    for assigned_group in assigned_groups:
        ...
        jql_customer_approval = jira.search_issues('project = ITSD AND status in ("Customer Approval") AND updated >= -8d AND updated <= 0d AND "Assigned group" = '+ str(assigned_group), maxResults=10000)
...
while True:
    ...
    #check_day
    if check_day == 1 and check_time == 11: #and check_time == 11:
       try:
    ...
    if check_day == 6 and check_time == 17:
        mass=[]
        today = date.today().strftime("%m")
        ...
        original = current_save_path+"\\\\"+old_file_name
        get_now = datetime.now()
        minus_6 = get_now - timedelta(days=5) #отнимаем дни до понедельника
        minus_6_day = datetime.strftime(minus_6, "%d-%m")
        date_now = datetime.strftime(get_now, "%d-%m-%Y")
        print(minus_6_day)
        print(date_now)
        global target
        target = current_save_path+"\\\\Еженедельный отчет "+str(minus_6_day)+"_"+str(date_now)+".docx" 
